...
struct EmailParser: CustomStringConvertible, ErrorGenerating, Codable {
    ...
    init(path: FilePath) throws {
        ...
        guard let contents = (try? path.string()) ?? nil else {
            throw EmailParser.error("processing", because: "The contents could not be loaded from \(path)")
        }
    ...
    init(contents: String) throws {
        ...
        guard let messageId = headers["message-id"] else {
            throw EmailParser.error("processing", because: "A 'message id' could not be found from \(headers["message-id"] ?? "NONE").")
        }

        guard let from = NamedEmailAddress(headers["from"]) else {
            throw EmailParser.error("processing", because: "A 'from' could not be found from \(headers["from"] ?? "NONE").")
        }

        if let toHeader = headers["to"] {
            guard let to = NamedEmailAddress.addresses(from: toHeader) else {
                throw EmailParser.error("processing", because: "An invalid 'to' was found from \(toHeader).")
            }
            self.to = to
        }
        else {
        ...
        guard let date = EmailParser.date(from: headers["date"]) else {
            throw EmailParser.error("processing", because: "An invalid 'date' was found from \(headers["date"] ?? "NONE").")
        }
        ...
        (self.content, self.attachments) = try EmailParser.content(
            from: body,
...
extension EmailParser.Attachment {
...
extension EmailParser.Content: Codable {
